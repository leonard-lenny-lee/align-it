...
FunctionMapping::FunctionMapping(Pharmacophore *ref, Pharmacophore *db,
                                 double eps) {
    // Initial values
    _hasNext = true;
    _maxLevel = 0;
    ...
    // Create an initial set of corresponding functional groups from reference
    // and database
    ...
    for (unsigned int i(0); i < _ref->size(); ++i) {
        ...
        // If we get here in the loop, the number of properties should be
        // counted
        initCounts = true;
    }
    ...
    for (unsigned int i(0); i < n; ++i) {
        std::vector<unsigned int> v(
            1); // create a new vector to store the first pairs
        v[0] = i;
    ...
    for (unsigned int i(0); i < n - 1; ++i) {
        ...
        for (unsigned int j(i + 1); j < n; ++j) {
            if ((_refIndex[i] == _refIndex[j]) ||
                (_dbIndex[i] == _dbIndex[j])) {
                // same group in one of the pairs, not possible
                continue;
            }
            // compute distances between functional groups within a molecule
            double d1 = distance((*_ref)[_refIndex[i]].point,
                                 (*_ref)[_refIndex[j]].point);
            double d2 =
                distance((*_db)[_dbIndex[i]].point, (*_db)[_dbIndex[j]].point);

            // check if the relative overlap between points is large enough
            d1 = (d1 - d2) * (d1 - d2);
            double o1 =
                GCI2 *
                pow(PI / ((*_ref)[_refIndex[i]].alpha +
                          (*_db)[_dbIndex[i]].alpha),
                    1.5) *
                exp(-((*_ref)[_refIndex[i]].alpha * (*_db)[_dbIndex[i]].alpha) *
                    d1 /
                    ((*_ref)[_refIndex[i]].alpha + (*_db)[_dbIndex[i]].alpha));
            double o2 =
                GCI2 *
                pow(PI / ((*_ref)[_refIndex[j]].alpha +
                          (*_db)[_dbIndex[j]].alpha),
                    1.5) *
                exp(-((*_ref)[_refIndex[j]].alpha * (*_db)[_dbIndex[j]].alpha) *
                    d1 /
                    ((*_ref)[_refIndex[j]].alpha + (*_db)[_dbIndex[j]].alpha));
            double v3 = GCI * pow(PI / (*_ref)[_refIndex[j]].alpha, 1.5);
            double v4 = GCI * pow(PI / (*_db)[_dbIndex[j]].alpha, 1.5);
            if ((o2 / (v3 + v4 - o2) > _epsilon) ||
                (o1 / (v1 + v2 - o1) > _epsilon)) {
    ...
    for (unsigned int level(3); level <= _maxLevel; ++level) {
        ...
        for (vi = _matchMap[level - 1]->begin();
             vi != _matchMap[level - 1]->end(); ++vi) {
            ...
            for (vj = _matchMap[2]->begin(); vj != _matchMap[2]->end(); ++vj) {
                ...
                for (unsigned int i(0); i < level - 2; ++i) {
                    bool found = false;
                    for (vk = _matchMap[2]->begin(); vk != _matchMap[2]->end();
                         ++vk) {
                        if ((*vk)[0] < (*vi)[i]) {
                            continue;
                        }
                        ...
                        if ((*vk)[1] == (*vj)[1]) { // found corresponding pair
                            found = true;
                            break;
                        }
                    }
...
FunctionMapping::~FunctionMapping() {
    std::map<unsigned int, std::vector<std::vector<unsigned int>> *>::iterator
        mi;
...
PharmacophoreMap FunctionMapping::getNextMap() {
    PharmacophoreMap pm;
    ...
    std::vector<std::vector<unsigned int>>::iterator vi =
        _matchMap[_maxLevel]->begin();
