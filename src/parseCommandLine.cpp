...
Options parseCommandLine(int argc, char *argv[]) {
    ...
    while ((choice = getopt_long(argc, argv, "vhqr:d:s:o:p:f:e:m", Arguments,
                                 &optionIndex)) != -1) {
        switch (choice) {
        case 'v': //......................................................version
            o.version = true;
        ...
        case 'f': { //.................................................funcGroup
            ...
            for (itL = l.begin(); itL != l.end(); ++itL) {
                ...
                mainErr("Undefined functional Group. Only AROM, HDON, HACC, "
                        "LIPO and"
    ...
    if (optind == 1) {
        ...
        if (!o.refInpType.empty()) {
            ...
            if (o.refInpType != "phar" && o.refInpType != "sdf") {
                mainErr("RDKit implementation currently only supports SDF or "
                        "PHAR (refInpType)");
            }
#endif
        } else {
            ...
            if (reader->FormatFromExt(o.refInpFile.c_str()) != NULL) {
                o.refInpType =
                    reader->FormatFromExt(o.refInpFile.c_str())->GetID();
            } else {
                o.refInpType.clear();
            }
            delete reader;
        ...
        if (!o.dbInpType.empty()) {
            ...
            if (reader->FormatFromExt(o.dbInpFile.c_str()) != NULL) {
                o.dbInpType =
                    reader->FormatFromExt(o.dbInpFile.c_str())->GetID();
            } else {
                o.dbInpType.clear();
            }
            delete reader;
        ...
        if (!o.molOutType.empty()) {
...
#else
            if (o.molOutType != "sdf") {
                mainErr("RDKit implementation currently only supports SDF "
                        "(molOutType)");
            }
#endif
        } else {
            ...
            if (reader->FormatFromExt(o.molOutFile.c_str()) != NULL) {
                o.molOutType =
                    reader->FormatFromExt(o.molOutFile.c_str())->GetID();
            } else {
                o.molOutType.clear();
            }
            delete reader;
        ...
        if (!o.molOutFile.empty() && !o.molOutType.empty()) {
            ...
            o.molOutWriter->SetOutFormat(
                o.molOutWriter->FindFormat(o.molOutType));
        } else if (!o.molOutFile.empty()) {
            o.molOutWriter = new OpenBabel::OBConversion();
            o.molOutWriter->SetOutFormat(
                o.molOutWriter->FormatFromExt(o.molOutFile));
        }
#else
        if (!o.molOutFile.empty() && !o.molOutType.empty()) {
