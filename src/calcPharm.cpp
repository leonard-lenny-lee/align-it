...
void calcPharm(Molecule *m, Pharmacophore *p, const Options &uo) {
    if (uo.funcGroupVec[AROM]) {
        aromFuncCalc(m, p);
    }
    if (uo.funcGroupVec[HDON]) {
        hDonFuncCalc(m, p);
    }
    if (uo.funcGroupVec[HACC]) {
        hAccFuncCalc(m, p);
    }
    if (uo.funcGroupVec[LIPO]) {
        lipoFuncCalc(m, p);
    }
