...
Alignment::Alignment(PharmacophoreMap &fMap)
    : _refMap(), _dbMap(), _refCenter(), _dbCenter(), _refRotMat(3, 3, 0.0),
      _dbRotMat(3, 3, 0.0), _dCdq(4, 0.0), _d2Cdq2(4, 4, 0.0), _grad(4, 0.0),
      _AkA(fMap.size()), _nbrPoints(0), _nbrExcl(0) {
    // compute centers of the two pharmacophore sets
    PharmacophoreMap::iterator mi;
    unsigned int nbrMatch(0);

    // compute the centroid of the pharmacophore sets
    double V1(0.0), v1(0.0), V2(0.0), v2(0.0);
    for (mi = fMap.begin(); mi != fMap.end(); ++mi) {
        if (mi->first->func == EXCL) {
            _nbrExcl++;
            continue; // exclusion spheres not considered
        }
        _nbrPoints++;
    ...
    // use SVD to compute the best set of rotations
    // set lower triangle
    mass1[1][0] = mass1[0][1];
    mass1[2][0] = mass1[0][2];
    mass1[2][1] = mass1[1][2];
    ...
    // check if determinant is 1, otherwise it is a mirroring instead of
    // rotation
    ...
    for (unsigned int i(0); i < _refMap.size(); ++i) {
        ...
        _refMap[i].point.x =
            _refRotMat[0][0] * x + _refRotMat[1][0] * y + _refRotMat[2][0] * z;
        _refMap[i].point.y =
            _refRotMat[0][1] * x + _refRotMat[1][1] * y + _refRotMat[2][1] * z;
        _refMap[i].point.z =
            _refRotMat[0][2] * x + _refRotMat[1][2] * y + _refRotMat[2][2] * z;

        x = _dbMap[i].point.x;
        y = _dbMap[i].point.y;
        z = _dbMap[i].point.z;
        _dbMap[i].point.x =
            _dbRotMat[0][0] * x + _dbRotMat[1][0] * y + _dbRotMat[2][0] * z;
        _dbMap[i].point.y =
            _dbRotMat[0][1] * x + _dbRotMat[1][1] * y + _dbRotMat[2][1] * z;
        _dbMap[i].point.z =
            _dbRotMat[0][2] * x + _dbRotMat[1][2] * y + _dbRotMat[2][2] * z;
...
SolutionInfo Alignment::align(bool n) {
    ...
    // try four different start orientations
    for (unsigned int _call(0); _call < 4; ++_call) {
        ...
        SiMath::Matrix hessian(4, 4, 0.0),
            dH(4, 4, 0.0); // hessian and hessian update
        unsigned int ii(0);
        for (; ii < 100; ++ii) {
            ...
            for (unsigned int i(0); i < _refMap.size(); ++i) {
                ...
                Aq[0] = (*AkA)[0][0] * rotor[0] + (*AkA)[0][1] * rotor[1] +
                        (*AkA)[0][2] * rotor[2] + (*AkA)[0][3] * rotor[3];
                Aq[1] = (*AkA)[1][0] * rotor[0] + (*AkA)[1][1] * rotor[1] +
                        (*AkA)[1][2] * rotor[2] + (*AkA)[1][3] * rotor[3];
                Aq[2] = (*AkA)[2][0] * rotor[0] + (*AkA)[2][1] * rotor[1] +
                        (*AkA)[2][2] * rotor[2] + (*AkA)[2][3] * rotor[3];
                Aq[3] = (*AkA)[3][0] * rotor[0] + (*AkA)[3][1] * rotor[1] +
                        (*AkA)[3][2] * rotor[2] + (*AkA)[3][3] * rotor[3];
                double qAq = Aq[0] * rotor[0] + Aq[1] * rotor[1] +
                             Aq[2] * rotor[2] + Aq[3] * rotor[3];
                v = GCI2 * pow(PI / (_refMap[i].alpha + _dbMap[i].alpha), 1.5) *
                    exp(-qAq);
                double c(1.0);

                // add normal if AROM-AROM in this case the absolute value of
                // the angle is needed
                if (n && (_refMap[i].func == AROM) &&
                    (_dbMap[i].func == AROM) && (_refMap[i].hasNormal) &&
                    (_dbMap[i].hasNormal)) {
                    // for aromatic rings only the planar directions count
                    // therefore the absolute value of the cosine is taken
                    c = _normalContribution(_refMap[i].normal, _dbMap[i].normal,
                                            rotor);
                    ...
                    for (unsigned int hi(0); hi < 4; hi++) {
                        ...
                        for (unsigned int hj(0); hj < 4; hj++) {
                            hessian[hi][hj] +=
                                v *
                                (_d2Cdq2[hi][hj] - 2.0 * _dCdq[hi] * Aq[hj] +
                                 2.0 * c *
                                     (2.0 * Aq[hi] * Aq[hj] - (*AkA)[hi][hj]));
                        }
                    }
                    v *= c;
                } else if (n &&
                    ...
                    // scale volume overlap of exclusion sphere with a negative
                    // scaling factor
            ...
            // stop iterations if the increase in volume overlap is too small
            // (gradient ascent) or if the volume is not defined
...
double Alignment::_normalContribution(Coordinate &n1, Coordinate &n2,
                                      SiMath::Vector &q) {
    double x = n2.x;
    double y = n2.y;
    double z = n2.z;
    ...
    // gradient update
    _dCdq[0] = _d2Cdq2[0][0] * q[0] + _d2Cdq2[0][1] * q[1] +
               _d2Cdq2[0][2] * q[2] + _d2Cdq2[0][3] * q[3];
    _dCdq[1] = _d2Cdq2[1][0] * q[0] + _d2Cdq2[1][1] * q[1] +
               _d2Cdq2[1][2] * q[2] + _d2Cdq2[1][3] * q[3];
    _dCdq[2] = _d2Cdq2[2][0] * q[0] + _d2Cdq2[2][1] * q[1] +
               _d2Cdq2[2][2] * q[2] + _d2Cdq2[2][3] * q[3];
    _dCdq[3] = _d2Cdq2[3][0] * q[0] + _d2Cdq2[3][1] * q[1] +
               _d2Cdq2[3][2] * q[2] + _d2Cdq2[3][3] * q[3];

    // return cosine
    return n1.x * Ux + n1.y * Uy + n1.z * Uz;
}

double Alignment::_quatVolumeOverlap(double alpha1, double alpha2,
                                     const SiMath::Vector &q,
