...
class Alignment {
  ...
  private:
    std::vector<PharmacophorePoint> _refMap;
    std::vector<PharmacophorePoint> _dbMap;
    ...
    // void _normalGradientMatrix(Coordinate &n1, Coordinate &n2, SiMath::Matrix
    // &A);
    double _quatVolumeOverlap(double alpha1, double alpha2,
                              const SiMath::Vector &q, const SiMath::Matrix &A);
    double _normalContribution(Coordinate &n1, Coordinate &n2,
                               SiMath::Vector &q);
};

#endif //__SILICOSIT_ALIGNIT_ALIGNMENT_H__
